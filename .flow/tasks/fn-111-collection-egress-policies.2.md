...
- Added adversarial mutation, diagnostic-init, malformed-counter, and late-stream redaction fixtures plus a live local HTTPS test proving IP rewrite with the original DNS certificate, sanitized successful response projection, and wrong-SAN rejection under process/caller verification overrides.
...
## Evidence
- Commits: b46797e7, 3ba27107, 47e7f589, 78e6992d
- Tests: bun test test/egress/destination-classifier.test.ts test/egress/destination-http-security.test.ts test/egress/http-policy-redaction.test.ts test/egress/policy.test.ts test/mcp/http-security.test.ts (42 pass, 0 fail), bun test test/egress test/mcp test/llm test/serve (766 pass, 0 fail), live local HTTPS fixture: pinned IP verified original model.test certificate, scrubbed successful Response.url, streamed the response body, and rejected wrong SAN despite process/caller TLS overrides, adversarial fixtures: public classification mutation cannot bypass redirect enforcement; request diagnostics and unknown Bun options are stripped; invalid redirect counts deny before DNS; initial fetch and late body read/cancel failures expose no IP, hostname, path, query, or credentials, bun run lint:check (0 warnings, 0 errors; formatting current), .flow/bin/flowctl validate --spec fn-111-collection-egress-policies --json (valid, 0 errors, 0 warnings), git diff --check (clean)
- PRs: