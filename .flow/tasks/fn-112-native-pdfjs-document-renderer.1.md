...
### Approach
- Baseline note: the durable baseline receipt `.flow/reviews/fn-112-landing-record.md` (created by task .2 step 0; raw logs at `/tmp/fn112-baseline/` are scratch) is the comparison target for this task's gates — compare against its enumerated pre-existing failure list and cite the `capture_id` used. Do not re-record the baseline here; if the receipt is missing or unverifiable, task .2 regenerates it via the isolated worktree path rather than this task inventing a new one.
...
- [ ] `bun test test/serve/api-doc-assets.test.ts test/serve/security.test.ts` green; `bun run lint:check` + `bunx tsc --noEmit` show no new failures vs the durable baseline receipt `.flow/reviews/fn-112-landing-record.md` (task .2 step 0)
...
- [ ] Focused suites + lint + typecheck: no new failures vs the durable baseline receipt `.flow/reviews/fn-112-landing-record.md` (task .2 step 0)
