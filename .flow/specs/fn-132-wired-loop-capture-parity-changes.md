...
3. **Scheduled findings pass.** `gno daemon` gains an opt-in scheduled task that runs the existing read-only audit (and saved-Capsule reverification) on a configurable cadence and writes findings as records into a dedicated findings collection — queryable through normal retrieval. Report-only: it never repairs, never mutates sources. Off by default.
