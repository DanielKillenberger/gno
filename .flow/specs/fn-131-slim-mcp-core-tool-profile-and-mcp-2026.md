...
- **Protocol dual-speak:** REQUIRES the @modelcontextprotocol/sdk v2 migration (repo pins 1.30.0, whose support stops at 2025-11-25; the official support-2026-07-28 migration guide governs). Scope includes package.json/bun.lock, every sdk import, scripts, and affected tests. 2025-11-25 clients keep working byte-for-byte; 2026-07-28 clients negotiate natively (server/discover, sessionless Streamable HTTP). The modern sessionless path MUST route through the same auth, write-gate, egress, admission, authorization-epoch, identity-isolation, and metrics enforcement as the existing HTTP boundary. Resident gateway profile config: key `gateway.toolProfile` (HttpGatewayConfigSchema mounts under root `gateway`), serve/daemon CLI override `--mcp-tool-profile`, CLI flag precedence over config over default `full`, applied at listener start. Core write allowlist with --enable-write is exact: gno_capture, gno_remember, plus gno_job_status iff an exposed write is async. `full` byte-compat covers descriptions: core uses separate description variants; full serves original strings verbatim.
