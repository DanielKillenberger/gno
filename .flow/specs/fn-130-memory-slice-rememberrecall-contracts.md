...
- **Scope enforcement is core correctness code:** every remember/recall call names explicit scopes; unscoped calls fail validation with a clear message. No implicit global scope. Shared scope is opt-in configuration, not default behavior.
...
- Input: fact text; collection; explicit scope(s); caller + session identity; decision `add` | `supersede`; for supersede: predecessor URI + content hash; optional recall receipt and source evidence.
