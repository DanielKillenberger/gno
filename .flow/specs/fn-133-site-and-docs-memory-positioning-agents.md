...
3. **qmd comparison refresh** (already stale: qmd 2.x has generate, HTTP MCP, structured_search, MCP 2026-07-28) plus the honest new lines: slim tool profile, protocol currency, and the memory split (qmd is deliberately read-only; GNO ships the write/memory contracts). Update the GBrain comparison's memory section to reference shipped remember/recall instead of future tense.
