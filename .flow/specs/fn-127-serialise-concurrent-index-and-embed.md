...
The important framing: **gno already solved this problem once.** `src/core/file-lock.ts` provides cross-process advisory locking (`lockf`/`flock` with a `BEGIN IMMEDIATE` SQLite fallback), and the MCP write tools take it -- capture, add-collection, remove-collection, clear-collection-embeddings, trace. The CLI write path does not. So the fix is not new machinery; it is applying the existing guard to the commands that skipped it, and making contention legible when it happens.
...
Design shape: a single **write-lease** wrapper around the CLI mutating commands, taken before the storage layer is opened and released in a `finally`. Lease is per-index-database, not per-collection, because the contention is on one shared `index-default.sqlite` across all collections. Waiting is the default behavior; failing is opt-in.
...
- **Resident writers.** `gno mcp --enable-write`, `gno serve`, and `gno daemon` hold the database for long periods. The lease must interoperate with the MCP tools' existing `withWriteLock` usage -- one lease namespace, not two competing ones. If MCP tools and the CLI take *different* locks, this spec has failed.
...
- **R1:** Every mutating CLI command acquires the shared write lease before opening the index for write and releases it on all exit paths, including thrown errors and signals. Errors: lease acquisition failure surfaces per R3; release failure is logged and never masks the command's own error.
...
- **R9:** CLI and MCP write paths contend on the same lease namespace -- an MCP write tool holding the lock blocks a CLI writer and vice versa. Errors: no error surface beyond R3.
...
- **R11:** Documentation states the concurrency model -- one writer at a time, readers always proceed, wait is the default, `--no-wait` opts out -- and notes that external serialising wrappers are no longer required. Errors: none.
