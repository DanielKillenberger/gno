...
- `GET /api/capabilities` reports `localClient`, true only for a loopback
  ...
  **Open original** for any document with a source file as an inline `/api/doc-asset` tab instead, and
...
- Split the production WebUI first JavaScript file into chunks, lazy-load
  ...
  is not either bar; this is not a 200ms TTI claim. Thanks @DanielKillenberger
  (#200).
...
- Corrected the Context Capsule file-output example to request canonical JSON
  explicitly before passing the saved capsule to verification.
...
- Recorded an immutable no-ship decision for CJK lexical normalization: the
  current diagnostics miss the frozen Chinese quality gates, so no production
  analyzer, schema, or support claim was introduced.
...
- Allowed the packed resident smoke to recognize a fully settled model-load
  recovery while still rejecting new failures, load churn, leaked leases, and
  active or in-flight work during warm-reuse proof.
...
- Added explicit verified Ask across CLI, REST, MCP, and SDK. It generates
  against one closed Context Capsule, preserves exact evidence spans and
  freshness state, classifies every substantive claim, and abstains below 100%
  support coverage.
- Added a separate attributable verified Ask outcome lane that executes
  production raw Ask and production `buildVerifiedAsk` over 22 paired fixture
  tasks. It requires no answer-accuracy loss and strictly fewer unsupported
  substantive claims, while preserving the existing Capsule retrieval
  promotion unchanged.
...
- Added a deterministic Chinese, Japanese, and Korean lexical-degradation
  benchmark with provenance-documented fixtures, per-language retrieval and
  latency metrics, concrete failure evidence, and immutable promotion gates for
  the planned CJK analyzer work. Public documentation now distinguishes
  semantic multilingual quality from model-free lexical fallback.
...
- Propagated configured global, collection, and path-scoped guidance consistently
  through BM25, vector, hybrid, Ask, REST, MCP, and SDK retrieval surfaces.
- Hardened grounded-answer prompt assembly against placeholder reparsing and
  forged source delimiters while keeping configured guidance separate from
  citable evidence.
...
- Recorded the July 2026 Nemotron 3 Embed comparison and current Qwen embedding
  candidate screen. GNO keeps `Qwen3-Embedding-0.6B-Q8_0` as the local default;
  Qwen3-VL-Embedding remains a future multimodal candidate.
...
- Restored `gno mcp serve --help` so the explicit MCP server subcommand shows normal help output without affecting stdio server startup.

## [1.5.0] - 2026-05-09

### Added

- Added graph-aware hybrid retrieval, graph edge confidence/audit metadata, graph health reports, CLI graph traversal, MCP graph neighbor/path tools, and Web UI community filtering.

## [1.4.2] - 2026-04-26

### Changed

- Updated `minimatch` from 10.1.1 to 10.2.3 and regenerated `bun.lock` so frozen installs stay reproducible.

## [1.4.1] - 2026-04-24

### Added

- Added `gno bench <fixture>` for local retrieval benchmark fixtures, including terminal and JSON output, fixture/result schemas, BM25/vector/hybrid mode comparisons, IR metrics, and an example fixture.
...
[1.27.1]: https://github.com/gmickel/gno/compare/v1.27.0...v1.27.1
[1.27.0]: https://github.com/gmickel/gno/compare/v1.26.0...v1.27.0
[1.26.0]: https://github.com/gmickel/gno/compare/v1.25.1...v1.26.0
[1.25.1]: https://github.com/gmickel/gno/compare/v1.25.0...v1.25.1
[1.25.0]: https://github.com/gmickel/gno/compare/v1.24.0...v1.25.0
[1.24.0]: https://github.com/gmickel/gno/compare/v1.23.0...v1.24.0
...
[1.12.1]: https://github.com/gmickel/gno/compare/v1.12.0...v1.12.1
[1.12.0]: https://github.com/gmickel/gno/compare/v1.11.0...v1.12.0
...
[1.5.1]: https://github.com/gmickel/gno/compare/v1.5.0...v1.5.1
[1.5.0]: https://github.com/gmickel/gno/compare/v1.4.2...v1.5.0
[1.4.2]: https://github.com/gmickel/gno/compare/v1.4.1...v1.4.2
[1.4.1]: https://github.com/gmickel/gno/compare/v1.4.0...v1.4.1
